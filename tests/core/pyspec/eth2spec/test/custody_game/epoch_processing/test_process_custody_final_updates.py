...
def test_validator_withdrawal_reenable_after_custody_reveal(spec, state):
    ...
    while state.validators[0].next_custody_secret_to_reveal <= spec.get_custody_period_for_validator(
...
def test_validator_withdrawal_suspend_after_chunk_challenge(spec, state):
    ...
    while state.validators[validator_index].next_custody_secret_to_reveal <= spec.get_custody_period_for_validator(
