...
from eth2spec.utils.ssz.ssz_typing import uint, Container, List, boolean, Vector, ByteVector, ByteList, Union, View
    ...
    if issubclass(typ, (uint, boolean)):
        ...
        for field_name, field_type in typ.fields().items():
            ...
            if field_name + "_hash_tree_root" in data:
                assert data[field_name + "_hash_tree_root"][2:] == hash_tree_root(temp[field_name]).hex()
        ret = typ(**temp)
