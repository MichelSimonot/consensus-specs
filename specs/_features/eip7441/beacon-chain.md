...
```python
def BLSG1ScalarMultiply(
    scalar: BLSFieldElement, point: BLSG1Point
) -> BLSG1Point:
...
def IsValidWhiskOpeningProof(
    ...
    tracker_proof: WhiskTrackerProof,
) -> bool:
    """
    Verify knowledge of `k` such that `tracker.k_r_G == k * tracker.r_G` and `k_commitment == k * BLS_G1_GENERATOR`.
    Defined in https://github.com/nalinbhardwaj/curdleproofs.pie/blob/dev/curdleproofs/curdleproofs/whisk_interface.py.
    """
    return curdleproofs.IsValidWhiskOpeningProof(
        tracker, k_commitment, tracker_proof
    )
```
...
class BeaconState(Container):
    ...
    whisk_trackers: List[
        WhiskTracker, VALIDATOR_REGISTRY_LIMIT
    ]  # [New in EIP7441]
    whisk_k_commitments: List[
        BLSG1Point, VALIDATOR_REGISTRY_LIMIT
    ]  # [New in EIP7441]
```
...
def select_whisk_proposer_trackers(state: BeaconState, epoch: Epoch) -> None:
    ...
    for i in range(PROPOSER_TRACKERS_COUNT):
        index = compute_shuffled_index(
            uint64(i),
            uint64(len(state.whisk_candidate_trackers)),
            proposer_seed,
        )
...
def select_whisk_candidate_trackers(state: BeaconState, epoch: Epoch) -> None:
    ...
    for i in range(CANDIDATE_TRACKERS_COUNT):
        ...
        state.whisk_candidate_trackers[i] = state.whisk_trackers[
            candidate_index
        ]
```
...
```python
def process_whisk_opening_proof(
...
def get_shuffle_indices(randao_reveal: BLSSignature) -> Sequence[uint64]:
    ...
    for i in range(0, VALIDATORS_PER_SHUFFLE):
        ...
        shuffle_index = (
            bytes_to_uint64(hash(pre_image)[0:8]) % CANDIDATE_TRACKERS_COUNT
        )
...
```python
def is_k_commitment_unique(
    state: BeaconState, k_commitment: BLSG1Point
) -> bool:
    return all(
        [
...
```python
def process_whisk_registration(
    state: BeaconState, body: BeaconBlockBody
) -> None:
...
def get_initial_whisk_k(
    ...
    return BLSFieldElement(
        bytes_to_bls_field(
            hash(
                uint_to_bytes(validator_index) + uint_to_bytes(uint64(counter))
            )
        )
    )
```
