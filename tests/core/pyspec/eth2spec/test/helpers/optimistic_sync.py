...
def add_optimistic_block(
    spec, mega_store, signed_block, test_steps, payload_status=None, status=PayloadStatusV1Status.SYNCING
):
    """
    ...
    yield from add_block(
        spec, mega_store.fc_store, signed_block, valid=valid, test_steps=test_steps, is_optimistic=True
    )
...
def get_opt_head_block_root(spec, mega_store):
    ...
    while True:
        children = [
            ...
            if (blocks[root].parent_root == head and not is_invalidated(mega_store, root))  # For optimistic sync
        ]
        if len(children) == 0:
            return head
