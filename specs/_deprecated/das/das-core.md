...
```python
def construct_proofs(
    extended_data_as_poly: Sequence[Point],
) -> Sequence[BLSCommitment]:
    """
...
def sample_data(
    ...
    return [
        DASSample(
            ...
            data=extended_data[
                i * POINTS_PER_SAMPLE : (i + 1) * POINTS_PER_SAMPLE
            ],
        )
...
```python
def verify_sample(
    sample: DASSample, sample_count: uint64, commitment: BLSCommitment
):
