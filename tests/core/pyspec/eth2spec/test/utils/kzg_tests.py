...
CELL_RANDOM_VALID1 = b"".join(
    [field_element_bytes(pow(2, n + 256, spec.BLS_MODULUS)) for n in range(spec.FIELD_ELEMENTS_PER_CELL)]
)
CELL_RANDOM_VALID2 = b"".join(
    [field_element_bytes(pow(3, n + 256, spec.BLS_MODULUS)) for n in range(spec.FIELD_ELEMENTS_PER_CELL)]
)
CELL_RANDOM_VALID3 = b"".join(
    [field_element_bytes(pow(5, n + 256, spec.BLS_MODULUS)) for n in range(spec.FIELD_ELEMENTS_PER_CELL)]
)
