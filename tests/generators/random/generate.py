...
def _generate_randomized_scenarios(block_randomizer):
    ...
    randomized_skips = (rng.sample(all_skips, len(all_skips)) for _ in range(BLOCK_TRANSITIONS_COUNT))
    ...
    scenarios = [{"transitions": _flatten(t)} for t in itertools.product(leak_transitions, block_transitions)]
