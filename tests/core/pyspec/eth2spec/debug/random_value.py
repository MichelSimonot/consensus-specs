...
def get_random_ssz_object(
    rng: Random, typ: Type[View], max_bytes_length: int, max_list_length: int, mode: RandomizationMode, chaos: bool
) -> View:
    """
    ...
    if issubclass(typ, ByteVector):
        ...
        return typ(
            get_random_ssz_object(rng, elem_type, max_bytes_length, max_list_length, mode, chaos) for _ in range(length)
        )
