...
class Validator(sharding.Validator):
    ...
    all_custody_secrets_revealed_epoch: (
        Epoch  # to be initialized to FAR_FUTURE_EPOCH
    )
```
...
class BeaconState(sharding.BeaconState):
    ...
    exposed_derived_secrets: Vector[
        List[
            ValidatorIndex, MAX_EARLY_DERIVED_SECRET_REVEALS * SLOTS_PER_EPOCH
        ],
...
def get_custody_secrets(key: BLSSignature) -> Sequence[int]:
    ...
    secrets = [
        int.from_bytes(
            signature_bytes[i : i + BYTES_PER_CUSTODY_ATOM], "little"
        )
...
```python
def process_custody_game_operations(
    state: BeaconState, body: BeaconBlockBody
) -> None:
    def for_ops(
    ...
    for_ops(
        body.early_derived_secret_reveals, process_early_derived_secret_reveal
    )
...
def process_chunk_challenge(
    ...
    if responder.exit_epoch < FAR_FUTURE_EPOCH:
        assert (
            ...
            <= responder.exit_epoch + MAX_CHUNK_CHALLENGE_DELAY
        )
    ...
    data_root = challenge.shard_transition.shard_data_roots[
...
```python
def process_custody_key_reveal(
    state: BeaconState, reveal: CustodyKeyReveal
) -> None:
    """
...
def process_early_derived_secret_reveal(
    ...
    if (
        reveal.epoch
        >= get_current_epoch(state) + CUSTODY_PERIOD_TO_RANDAO_PADDING
    ):
        ...
        proposer_reward = Gwei(
            whistleblowing_reward // PROPOSER_REWARD_QUOTIENT
        )
...
def process_custody_slashing(
    ...
    domain = get_domain(
        state, DOMAIN_CUSTODY_BIT_SLASHING, get_current_epoch(state)
    )
    ...
    assert (
        hash_tree_root(shard_transition)
...
def process_reveal_deadlines(state: BeaconState) -> None:
    ...
    for index, validator in enumerate(state.validators):
        ...
        if (
            get_custody_period_for_validator(ValidatorIndex(index), epoch)
            > deadline
        ):
...
def process_challenge_deadlines(state: BeaconState) -> None:
    for custody_chunk_challenge in state.custody_chunk_challenge_records:
        if (
            get_current_epoch(state)
            > custody_chunk_challenge.inclusion_epoch
            + EPOCHS_PER_CUSTODY_PERIOD
        ):
            slash_validator(
                state,
