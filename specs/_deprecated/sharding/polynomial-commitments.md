...
def roots_of_unity(order: uint64) -> List[BLSFieldElement]:
    ...
    for i in range(SAMPLES_PER_BLOB * FIELD_ELEMENTS_PER_SAMPLE):
        ...
        current_root_of_unity = (
            current_root_of_unity * root_of_unity % BLS_MODULUS
        )
    return roots
```
...
def add_polynomials(
    ...
    return [
        (a[i] + (b[i] if i < len(b) else 0)) % BLS_MODULUS
...
```python
def hash_to_bls_field(
    x: Container, challenge_number: uint64
) -> BLSFieldElement:
    """
    ...
    return (
        int.from_bytes(
            hash(
                hash_tree_root(x)
                + int.to_bytes(challenge_number, 32, "little")
            ),
            "little",
        )
    ) % BLS_MODULUS
