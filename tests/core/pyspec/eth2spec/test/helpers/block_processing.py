...
def get_process_calls(spec):
    return {
        ...
        "process_attestation": lambda state, block: for_ops(state, block.body.attestations, spec.process_attestation),
        "process_deposit": lambda state, block: for_ops(state, block.body.deposits, spec.process_deposit),
        "process_voluntary_exit": lambda state, block: for_ops(
            state, block.body.voluntary_exits, spec.process_voluntary_exit
        ),
        # Altair
        "process_sync_aggregate": lambda state, block: spec.process_sync_aggregate(state, block.body.sync_aggregate),
        # Bellatrix
        "process_application_payload": lambda state, block: spec.process_application_payload(state, block.body),
        # TODO: add sharding processing functions when spec stabilizes.
        # Custody Game
        "process_custody_game_operations": lambda state, block: spec.process_custody_game_operations(state, block.body),
    }
