...
def container_case_fn(rng: Random, mode: RandomizationMode, typ: Type[View], chaos: bool = False):
    return get_random_ssz_object(rng, typ, max_bytes_length=2000, max_list_length=2000, mode=mode, chaos=chaos)
    ...
    for name, (typ, offsets) in PRESET_CONTAINERS.items():
        ...
        for mode in modes:
            for variation in range(10):
                yield f"{name}_{mode.to_name()}_{variation}", valid_test_case(lambda: container_case_fn(rng, mode, typ))


def mod_offset(b: bytes, offset_index: int, change: Callable[[int], int]):
    return (
        b[:offset_index]
        + (change(int.from_bytes(b[offset_index : offset_index + 4], byteorder="little")) & 0xFFFFFFFF).to_bytes(
...
def invalid_cases():
    ...
    for name, (typ, offsets) in PRESET_CONTAINERS.items():
        ...
        yield f"{name}_extra_byte", invalid_test_case(
            lambda: serialize(container_case_fn(rng, RandomizationMode.mode_max_count, typ)) + b"\xff"
        )
