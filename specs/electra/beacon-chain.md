...
```python
def is_compounding_withdrawal_credential(
    withdrawal_credentials: Bytes32,
) -> bool:
...
```python
def get_committee_indices(
    committee_bits: Bitvector,
) -> Sequence[CommitteeIndex]:
    return [
        CommitteeIndex(index)
        for index, bit in enumerate(committee_bits)
        if bit
    ]
```
...
def get_consolidation_churn_limit(state: BeaconState) -> Gwei:
    return get_balance_churn_limit(state) - get_activation_exit_churn_limit(
        state
    )
```
...
```python
def get_next_sync_committee_indices(
    state: BeaconState,
...
```python
def queue_excess_active_balance(
    state: BeaconState, index: ValidatorIndex
) -> None:
...
def compute_consolidation_epoch_and_update_churn(
    ...
    if consolidation_balance > consolidation_balance_to_consume:
        balance_to_process = (
            consolidation_balance - consolidation_balance_to_consume
        )
        additional_epochs = (
...
def get_expected_withdrawals(
    state: BeaconState,
) -> Tuple[Sequence[Withdrawal], uint64]:
    epoch = get_current_epoch(state)
