...
def basic_vector_case_fn(rng: Random, mode: RandomizationMode, elem_type: Type[BasicView], length: int):
    return get_random_ssz_object(
        rng, Vector[elem_type, length], max_bytes_length=length * 8, max_list_length=length, mode=mode, chaos=False
    )
        ...
        for length in [1, 2, 3, 4, 5, 8, 16, 31, 512, 513]:
            ...
            for mode in random_modes:
                if length == 1:
                    ...
                    yield f"vec_{name}_{length}_{mode.to_name()}_one_less", invalid_test_case(lambda: b"")
                else:
